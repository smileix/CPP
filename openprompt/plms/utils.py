...
class TokenizerWrapper:
    ...
    def padding(input_dict: Dict,
                max_len: int, pad_id_for_inputs: int=0, pad_id_for_others: int=0) -> dict:
        # assert len(input_dict['input_ids']) == len(input_dict['attention_mask'])
        seq_len = len(input_dict['input_ids'])
        pad_num = max_len - seq_len
        for key, value in input_dict.items():
            if pad_num != 0:
                zeros = torch.zeros(pad_num).long()
                if key in ["input_ids", "attention_mask", "soft_token_ids", "loss_ids"]:
                    if torch.is_tensor(value):
                        input_dict[key] = torch.cat((input_dict[key], zeros), 0).long()
                    else:
    ...
    # @staticmethod
    # def padding(input_dict: Dict, max_len: int, pad_id_for_inputs: int = 0, pad_id_for_others: int = 0) -> None:
    #     padding_dict = {}
    #     for key, value in input_dict.items():
    ...
    #
    # #         # todo 此处的判断还需要完善
    #         if key in ["input_ids", "attention_mask", "label", "soft_token_ids", "loss_ids", "guid"]:
    #             if torch.is_tensor(value):
    #                 padding_dict[key] = torch.cat((input_dict[key], zeros), 0).long()
    #             else:
    #                 padding_dict[key] = input_dict[key]
