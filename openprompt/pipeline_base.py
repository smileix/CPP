...
class PromptDataLoader(object):
    ...
    def tokenize(self) -> None:
        ...
        # for idx, wrapped_example in enumerate(self.wrapped_dataset):
            ...
            # print(inputfeatures.input_ids.shape)
            # print(inputfeatures.attention_mask.shape)
            # assert inputfeatures.input_ids.shape == inputfeatures.attention_mask.shape
...
class PromptForClassification(nn.Module):
    ...
    def forward(self, batch: Union[Dict, InputFeatures], input_prototype=False) -> torch.Tensor:
        r"""
        ...
        Returns:
            :param input_prototype: 用来判断输入的batch是否为prototype，如果是的话，处理逻辑会有区别
        ...
        # 下面这一步处理去掉了hidden states，只保留了logits
        # print(outputs.hidden_states)
        if not input_prototype:
            ...
            return label_words_logits, hidden_states
            # return label_words_logits, outputs_at_mask
