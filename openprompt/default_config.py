...
def get_default_config():
    ...
    # cfg.logging.unique_string_keys = ['dataset.name', 'plm.model_path', 'template', 'verbalizer', 'datetime']
    cfg.logging.unique_string_keys = ['dataset.name', 'datetime']
    # used to generate the unique string for saving
        #- dataset.name
    ...
    cfg.logging.datetime_format = "%m%d%H%M%S" # only useful when unique_string_keys includes `datetime`.
    ...
    cfg.checkpoint.save_latest = False # Normally set to False to reduce memory use, set
    ...
    cfg.checkpoint.patience = 10

    ## PIPELINE #######################################################

    cfg.train = CfgNode(new_allowed=True)
    cfg.train.num_epochs = 50 # the number of training epochs.
