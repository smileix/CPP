...
class InputFeatures(dict):
    ...
    def collate_fct(batch: List):
        ...
        # todo 在此处进行padding，全局最大长度为128，然后按照局部最大长度来padding。
        ## 先获取batch中所有输入的长度并算出局部最大长度，然后调用plm.utils中的padding方法，进行padding，或者重写该方法
        ...
        # print(batch_max_len)
        bsz = len(batch)
        batch = [TokenizerWrapper.padding(input_dict=batch[idx], max_len=batch_max_len) for idx in range(bsz)]

        # for elem in batch:
            # print(len(elem['input_ids']))
            # print(len(elem['attention_mask']))
            # assert len(elem['input_ids']) == len(elem['attention_mask'])
        ...
        for key in elem:
            if key in ["input_ids", "attention_mask", "soft_token_ids", "loss_ids"]:
                temp_list = [d[key] for d in batch]

                # for i in temp_list:
                #     assert temp_list[0].shape == i.shape, f"the length of {key} is not equal, {temp_list[0].shape} vs {i.shape}"

                return_dict[key] = torch.stack(temp_list)
            else:
                return_dict[key] = default_collate([d[key] for d in batch])
        ...
        # 在这之前的data都是以行为单位的，每行都是一条数据的所有信息，也就是input、mask与label，
        # 然后需要处理成以列为单位的，按照batch馈送给模型。
        # 比如说三列，分别是是一个batch的所有input，或者mask，或者label。
        ...
        #         temp_list = [d[key] for d in batch]
        #         return_dict[key] = torch.stack(temp_list)
        #     if key in ["label", "guid"]:
        #         return_dict[key] = [d[key] for d in batch]

            # # 此处定义枚举dataloader的返回值，将label由tensorable的list转为dict之后，需要在此处手动添加该返回值，在后面计算loss或者推理时调用
            # if key == "label":
            #     return_dict[key] = [d[key] for d in batch]
            # if key == 'encoded_tgt_text':
            #     return_dict[key] = [d[key] for d in batch]
