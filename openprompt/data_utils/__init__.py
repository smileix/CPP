...
def load_dataset(config: CfgNode, return_class=True, test=False):
    ...
    if not test:
        try:
            train_dataset = processor.get_train_examples(dataset_config.path, mode=config.train.mode if config.train.mode else None)
        except FileNotFoundError:
            logger.warning(f"Has no training dataset in {dataset_config.path}.")
        try:
            valid_dataset = processor.get_dev_examples(dataset_config.path, mode=config.train.mode if config.train.mode else None)
        except FileNotFoundError:
            logger.warning(f"Has no validation dataset in {dataset_config.path}.")

    test_dataset = None
    try:
        test_dataset = processor.get_test_examples(dataset_config.path, mode=config.train.mode if config.train.mode else None)
    except FileNotFoundError:
