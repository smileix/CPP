...
def init_logger(
    ...
    # log_format = logging.Formatter("[\033[032m%(asctime)s\033[0m %(levelname)s] %(module)s.%(funcName)s %(message)s", datefmt=date_format)
    log_format = logging.Formatter("[\033[032m%(asctime)s\033[0m %(levelname)s] %(message)s", datefmt=date_format)
    logger = logging.getLogger()
