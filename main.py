...
def main():
    ...
    if args.resume or args.test or args.discover or args.discover_test:
        config.logging.path = EXP_PATH = args.resume or args.test or args.discover
        init_logger(os.path.join(EXP_PATH, "log_discover.txt"), config.logging.file_level, config.logging.console_level)
    else:
        EXP_PATH = config_experiment_dir(config)
        init_logger(os.path.join(EXP_PATH, "log_pretrain.txt"), config.logging.file_level, config.logging.console_level)
...
def trainer(EXP_PATH, config, Processor, train_dataset = None, valid_dataset = None, test_dataset = None, resume = None, test = None, zero = False, discover = False):
    ...
    if zero:
        ...
        res = runner.test(ckpt = 'best_discover')
    elif resume:
        # 利用这个resume机制来控制discover开始的点
        res = runner.run(ckpt = 'last')
        # res = runner.run(ckpt = 'best')
    elif discover:
        res = runner.run(ckpt='best_pretrain')
    else:
        res = runner.run()
    return res
